...
void slabs_init(const size_t limit, const double factor, const bool prealloc) {
    ...
    unsigned int size = sizeof(CItem) + g_settings.chunk_size;
    ...
    while (++i < POWER_LARGEST && size <= g_settings.item_size_max / factor) {
    ...
    power_largest = i;
    slabclass[power_largest].size = g_settings.item_size_max;
...
static void *do_slabs_alloc(const size_t size, unsigned int id) {
    ...
    if (id < POWER_SMALLEST || (int)id > power_largest) {
        return NULL;
    }

    p = &slabclass[id];
    assert(p->sl_curr == 0 || ((CItem *)p->slots)->slabs_clsid == 0);
...
static void do_slabs_free(void *ptr, const size_t size, unsigned int id) {
    ...
    assert(((CItem *)ptr)->slabs_clsid == 0);
    assert(id >= POWER_SMALLEST && (int)id <= power_largest);
    if (id < POWER_SMALLEST || (int)id > power_largest)
        return;
...
static int nz_strcmp(size_t nzlength, const char *nz, const char *z) {
    size_t zlength=strlen(z);
    return (zlength == nzlength) && (strncmp(nz, z, zlength) == 0) ? 0 : -1;
}

bool get_stats(const char *stat_type, size_t nkey, CConn *c) {
    bool ret = true;

    if (!stat_type) {
        /* prepare general statistics for the engine */
        c->AppendStat("bytes", "%llu", (unsigned long long)g_stats.m_currBytes);
        c->AppendStat("curr_items", "%u", g_stats.m_currItems);
        c->AppendStat("total_items", "%u", g_stats.m_totalItems);
        item_stats_totals(c);
...
void CConn::DoSlabsStats()
{
    ...
    for(i = POWER_SMALLEST; i <= power_largest; i++) {
        ...
        if (p->slabs != 0) {
            ...
            APPEND_NUM_STAT(i, "get_hits", "%llu",
                    (unsigned long long)thread_stats.m_slabStats[i].m_getHits);
            APPEND_NUM_STAT(i, "cmd_set", "%llu",
                    (unsigned long long)thread_stats.m_slabStats[i].m_setCmds);
            APPEND_NUM_STAT(i, "delete_hits", "%llu",
                    (unsigned long long)thread_stats.m_slabStats[i].m_deleteHits);
            APPEND_NUM_STAT(i, "incr_hits", "%llu",
                    (unsigned long long)thread_stats.m_slabStats[i].m_incrHits);
            APPEND_NUM_STAT(i, "decr_hits", "%llu",
                    (unsigned long long)thread_stats.m_slabStats[i].m_decrHits);
            APPEND_NUM_STAT(i, "cas_hits", "%llu",
                    (unsigned long long)thread_stats.m_slabStats[i].m_casHits);
            APPEND_NUM_STAT(i, "cas_badval", "%llu",
                    (unsigned long long)thread_stats.m_slabStats[i].m_casBadVal);
            APPEND_NUM_STAT(i, "touch_hits", "%llu",
                    (unsigned long long)thread_stats.m_slabStats[i].m_touchHits);
            total++;
        }
    }
    ...
    AppendStat("active_slabs", "%d", total);
    AppendStat("total_malloced", "%llu", (unsigned long long)mem_malloced);
...
static int slab_rebalance_start(void) {
    ...
    g_stats.m_bSlabReassignRunning = true;
...
static int slab_rebalance_move(void) {
    ...
    for (x = 0; x < slab_bulk_check; x++) {
        CItem *it = (CItem *)slab_rebal.slab_pos;
        status = MOVE_PASS;
        if (it->slabs_clsid != 255) {
            void *hold_lock = NULL;
            uint32_t hv = it->hv;//hash(ITEM_key(it), it->nkey, 0);
...
static void slab_rebalance_finish(void) {
    ...
    memset(slab_rebal.slab_start, 0, (size_t)g_settings.item_size_max);
    ...
    g_stats.m_bSlabReassignRunning = false;
    __sync_fetch_and_add(&g_stats.m_slabsMoved, 1);
