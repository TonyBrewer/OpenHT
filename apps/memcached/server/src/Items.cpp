...
static void item_link_q(CItem *it);
static void item_unlink_q(CItem *it);
...
static CItem *heads[LARGEST_ID];
static CItem *tails[LARGEST_ID];
...
static size_t item_make_header(const uint8_t nkey, const int flags, const int nbytes,
    ...
    *nsuffix = (uint8_t) snprintf(suffix, 40, " %d %d\r\n", flags, nbytes - 2);
    return sizeof(CItem) + nkey + *nsuffix + nbytes;
}

/*@null@*/
CItem *do_item_alloc(char *key, const size_t nkey,
                    const uint32_t hv, const int flags,
                    const rel_time_t exptime, const int nbytes) {
    unsigned int id;
    uint8_t nsuffix;
    CItem *it = NULL;
    char suffix[40];
    size_t ntotal = item_make_header((uint8_t)(nkey + 1), flags, nbytes, suffix, &nsuffix);
    int tries = 5;
    int tried_alloc = 0;
    CItem *search;
    void *hold_lock = NULL;
    ...
    for (; tries > 0 && search != NULL; tries--, search=search->prev) {
        uint32_t shv = search->hv;		//hash(ITEM_key(search), search->nkey, 0);
        /* Attempt to hash item lock the "search" item. If locked, no
         * other callers can incr the refcount
         */
        /* FIXME: I think we need to mask the hv here for comparison? */
        if (shv != hv && (hold_lock = item_trylock(shv)) == NULL)
            continue;
        ...
        if ((search->exptime != 0 && search->exptime < current_time)
            ...
            it->slabs_clsid = 0;
        } else if ((it = (CItem *)slabs_alloc(ntotal, id)) == NULL) {
            tried_alloc = 1;
    ...
    if (!tried_alloc && (tries == 0 || search == NULL))
        it = (CItem *)slabs_alloc(ntotal, id);
...
bool item_size_ok(const size_t nkey, const int flags, const int nbytes) {
    ...
    size_t ntotal = item_make_header((uint8_t)(nkey + 1), flags, nbytes,
                                     prefix, &nsuffix);
    if (g_settings.use_cas) {
        ntotal += sizeof(uint64_t);
    }
...
static void item_link_q(CItem *it) { /* item is the new head */
    CItem **head, **tail;
...
int do_item_link(CItem *it) {
    assert((it->it_flags & (ITEM_LINKED|ITEM_SLABBED)) == 0);
    mutex_lock(&cache_lock);
    it->it_flags |= ITEM_LINKED;
    it->time = current_time;

    __sync_fetch_and_add(&g_stats.m_currBytes, ITEM_ntotal(it));
    __sync_fetch_and_add(&g_stats.m_currItems, 1);
    __sync_fetch_and_add(&g_stats.m_totalItems, 1);
...
void do_item_unlink(CItem *it) {
    ...
    if ((it->it_flags & ITEM_LINKED) != 0) {
        it->it_flags &= ~ITEM_LINKED;
	    __sync_fetch_and_sub(&g_stats.m_currBytes, ITEM_ntotal(it));
        __sync_fetch_and_sub(&g_stats.m_currItems, 1);
        assoc_delete(ITEM_key(it), it->nkey, it->hv);
        item_unlink_q(it);
        do_item_remove(it);
    }
    mutex_unlock(&cache_lock);
...
char *do_item_cachedump(const unsigned int slabs_clsid, const unsigned int limit, unsigned int *bytes) {
    ...
    while (it != NULL && (limit == 0 || shown < limit)) {
        ...
        len = snprintf(temp, sizeof(temp), "ITEM %s [%d b; %lu s]\r\n",
                       ...
                       (unsigned long)it->exptime + g_processStartTime);
...
void do_item_stats_totals(CConn *c) {
    ...
    c->AppendStat("expired_unfetched", "%llu",
                (unsigned long long)totals.expired_unfetched);
    c->AppendStat("evicted_unfetched", "%llu",
                (unsigned long long)totals.evicted_unfetched);
    c->AppendStat("evictions", "%llu",
                (unsigned long long)totals.evicted);
    c->AppendStat("reclaimed", "%llu",
                (unsigned long long)totals.reclaimed);
}

void CConn::DoItemStats() {
    int i;
    for (i = 0; i < LARGEST_ID; i++) {
        if (tails[i] != NULL) {
    ...
    if (histogram != NULL) {
        ...
        for (i = 0; i < num_buckets; i++) {
            if (histogram[i] != 0) {
                ...
                c->AppendStat(key, "%u", histogram[i]);
            }
        }
        free(histogram);
    }
    c->AddStats(NULL, 0, NULL, 0);
}
...
CItem *do_item_get(const char *key, const size_t nkey, const uint32_t hv) {
    //mutex_lock(&cache_lock);
    int was_found = 0;
    CItem *it = assoc_find(key, nkey, hv);
    if (it != NULL) {
        refcount_incr(&it->refcount);
    ...
    if (it != NULL) {
        if (g_settings.oldest_live != 0 && g_settings.oldest_live <= current_time &&
            it->time <= g_settings.oldest_live) {
            do_item_unlink(it);
            do_item_remove(it);
            it = NULL;
            if (was_found) {
...
CItem *do_item_touch(const char *key, const size_t nkey, uint32_t exptime,
                    const uint32_t hv) {
...
void do_item_flush_expired(void) {
    ...
    for (i = 0; i < LARGEST_ID; i++) {
        ...
        for (iter = heads[i]; iter != NULL; iter = next) {
            if (iter->time >= g_settings.oldest_live) {
                next = iter->next;
                if ((iter->it_flags & ITEM_SLABBED) == 0) {
                    do_item_unlink_nolock(iter);
                }
            } else {
