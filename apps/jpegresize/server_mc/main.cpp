...
int main(int argc, char **argv)
{
  ...
  rpc::server svr;
  resizeDispatcher dispatcher(appArguments.threads, appArguments.coprocs, appArguments.jobSlots, appArguments.responders);
  std::auto_ptr<rpc::dispatcher> dp(&dispatcher);
  ...
  svr.start(appArguments.listeners);
  ...
  while(1) {
    usleep(delay);
    if(appArguments.stats) {
      dispatcher.jobStats(seconds, jobCnt, jobActiveAvg, jobTimeAvg);
