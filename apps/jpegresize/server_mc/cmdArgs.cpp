...
bool parseArgs(int argc, char *argv[]) {
  ...
  while((opt=(cmdLineArg)getopt_long(argc, argv, shortOpts, longOpts, &longIndex ))!=-1) {
	...
	break;
      case argListeners:
	appArguments.listeners=strtol(optarg, &endptr,0);
	if(*endptr) {
	  printf("Invalid number of listeners: %s\n", optarg);
	  printHelp();
	  return false;	  
	} else if (appArguments.listeners==0) {
	  printf("--listeners requires a numeric argument\n");
	  printHelp();
	  return false;	  
	}
	break;
      case argCoprocUnits:
	appArguments.coprocs=strtol(optarg, &endptr,0);
	if(*endptr) {
	  printf("Invalid number of coprocessors: %s\n", optarg);
	  printHelp();
	  return false;	  
	} else if (appArguments.coprocs==0) {
	  printf("--coproc requires a numeric argument\n");
	  printHelp();
	  return false;	  
	}	
	break;
      case 	argJobSlots:
	appArguments.jobSlots=strtol(optarg, &endptr,0);
	if(*endptr) {
	  printf("Invalid number of job slots: %s\n", optarg);
	  printHelp();
	  return false;	  
	} else if (appArguments.jobSlots==0) {
	  printf("--job_slots requires a numeric argument\n");
	  printHelp();
	  return false;	  
	}	
	break;
      case 	argResponders:
	appArguments.responders=strtol(optarg, &endptr,0);
	if(*endptr) {
	  printf("Invalid number of coprocessors: %s\n", optarg);
	  printHelp();
	  return false;	  
	} else if (appArguments.responders==0) {
	  printf("--responders requires a numeric argument\n");
	  printHelp();
	  return false;	  
	}	
	break;	
...
void printHelp(void) {
  ...
  printf("     --threads <n>        number of threads to feed each coprocessor unit\n");
  printf("     --listeners <n>      number of RPC listeners to handle incoming requests\n");
  printf("     --responders <n>  number of RPC responders to handle outgoing responses\n");
  printf("     --coproc <n>         number of coprocessors to use\n");
  printf("     --job_slots <n>      number of jobs slots to allocate for each coprocessor\n");
...
void printArgs(void) {
  ...
  printf("       syslog: %d\n", appArguments.syslog);
  printf("     jobSlots: %d\n", appArguments.jobSlots);
  printf("responders: %d\n", appArguments.responders);
  return;
}
