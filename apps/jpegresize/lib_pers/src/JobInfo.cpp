...
JobInfo * JobInfo::m_pPoolHead = 0;
volatile int64_t JobInfo::m_lock;
uint8_t * JobInfo::m_pHifJobInfoBase;
uint8_t * JobInfo::m_pHifInPicBase;
uint8_t * JobInfo::m_pHifRstBase;
unsigned int JobInfo::m_threadIdx = 0;
unsigned int JobInfo::m_threadCnt;
bool * JobInfo::m_pThreadJobInfo = 0;
uint8_t * JobInfo::m_pThreadLog = 0;
int JobInfo::m_threadLogWrIdx = 0;

#define MAX_THREAD_LOG 1000000

void* JobInfo::operator new( size_t, int threadId, bool check, bool useHostMem, bool preAllocSrcMem)
{
	Lock();
	...
	if (m_pPoolHead == 0) {
		...
		for (int ci = 0; ci < 3; ci += 1) {
			if (check) {
				if (useHostMem) {
				  ...
				  m_pPoolHead->m_dec.m_dcp[ci].m_pCompBuf = (uint8_t*)g_pJpegResizeHif->MemAllocAlign(MAX_IMAGE_COMP_SIZE, MAX_IMAGE_COMP_SIZE);
		...
		if (useHostMem) {
		  ...
		  pCpMem = (uint8_t*)g_pJpegResizeHif->MemAllocAlign(MAX_IMAGE_COMP_SIZE, MAX_IMAGE_COMP_SIZE*3);
		  assert(pCpMem!=NULL);
		}
		...
		for (int ci = 0; ci < 3; ci += 1) {
			if (check) {
				if (useHostMem) {
				  ...
				  m_pPoolHead->m_vert.m_vcp[ci].m_pOutCompBuf = (uint8_t*)g_pJpegResizeHif->MemAllocAlign(MAX_IMAGE_COMP_SIZE, MAX_IMAGE_COMP_SIZE*3);
