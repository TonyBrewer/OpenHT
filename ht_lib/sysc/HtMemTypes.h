...
template <typename T, int addrBits> class ht_ultra_que {
public:
	ht_ultra_que() {
		ht_assert(addrBits < 18);
		m_maxEntriesUsed = 0;
	...
	friend void sc_trace(sc_trace_file *tf, const ht_ultra_que & v, const std::string & NAME ) {
	}
...
public:
	...
	void read_addr(uint64_t rdIdx1, uint64_t rdIdx2 = 0) const {
		ht_assert(m_bReset || !m_bRdAddr);
		const_cast<ht_ultra_ram*>(this)->m_bRdAddr = true;
		const_cast<ht_ultra_ram*>(this)->m_rdAddr = (rdIdx1 + (rdIdx2 << AW1)) & ((1 << AW1) * (1 << AW2) - 1);
	}
	uint64_t read_addr() { return m_rdAddr; }
	bool read_in_use() { return m_bRdAddr; }
	void write_addr(uint64_t wrIdx1, uint64_t wrIdx2=0) {
	...
	friend void sc_trace(sc_trace_file *tf, const ht_ultra_ram & v, const std::string & NAME ) {
	}
...
public:
	...
	friend void sc_trace(sc_trace_file *tf, const ht_mrd_ultra_ram & v, const std::string & NAME ) {
	}
...
public:
	...
	friend void sc_trace(sc_trace_file *tf, const ht_mwr_ultra_ram & v, const std::string & NAME ) {
	}
