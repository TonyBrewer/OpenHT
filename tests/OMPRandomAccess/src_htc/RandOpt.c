...
static void
RandomAccessUpdate(uint64_t TableSize, uint64_t *Table, int numt) {
    uint64_t i;
    uint64_t *ran;
    //  uint64_t ran[NUMT];              /* Current random numbers */
    int j;
...
#pragma omp target teams num_teams(unitCnt)
    {
        int unit = omp_get_team_num();
        uint64_t *unitran = ran + (unit * numt);
...
main (int argc, char **argv) {
  ...
  RandomAccessUpdate(TableSize,cp_Table, 256);
