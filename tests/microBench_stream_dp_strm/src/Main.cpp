...
int main(int argc, char **argv)
{
	...
	printf("Stream Microbenchmark (Streaming)\n");
	printf("  Running with vecLen = %llu, scalar = %lf\n", (long long)vecLen, scalar);
	printf("  Initializing arrays\n");
	fflush(stdout);

	// Initialize Arrays on Host
	a1 = (double *)malloc(vecLen * sizeof(double));
	a2 = (double *)malloc(vecLen * sizeof(double));
	a3 = (double *)malloc(vecLen * sizeof(double));
	a1Chk = (double *)malloc(vecLen * sizeof(double));
	a2Chk = (double *)malloc(vecLen * sizeof(double));
	a3Chk = (double *)malloc(vecLen * sizeof(double));
	...
	memcpy(a1Chk, a1, vecLen * sizeof(double));
	memcpy(a2Chk, a2, vecLen * sizeof(double));
	memcpy(a3Chk, a3, vecLen * sizeof(double));
	...
	double *cp_a3 = a3;
#else
	double *cp_a1 = (double*)pHtHif->MemAlloc(vecLen * sizeof(double));
	double *cp_a2 = (double*)pHtHif->MemAlloc(vecLen * sizeof(double));
	double *cp_a3 = (double*)pHtHif->MemAlloc(vecLen * sizeof(double));
	if (!cp_a1 || !cp_a2 || !cp_a3) {
		fprintf(stderr, "ht_cp_malloc() failed.\n");
		exit(-1);
	}
	pHtHif->MemCpy(cp_a1, a1, vecLen * sizeof(double));
	pHtHif->MemCpy(cp_a2, a2, vecLen * sizeof(double));
	pHtHif->MemCpy(cp_a3, a3, vecLen * sizeof(double));
#endif
	...
	for (int loop = 0; loop < NTIMES; loop++) {
		...
		for (int operation = 0; operation < 4; operation++) {
			...
			delta_usec(NULL);
			for (int unit = 0; unit < unitCnt; unit++) {
			  pAuUnits[unit]->SendCall_htmain(operation, *(uint64_t*)&scalar, unitOffset, unitVecLen);
				unitOffset += unitVecLen;
				unitVecLen = (unitVecLen < (vecLen - unitOffset)) ? unitVecLen : (vecLen - unitOffset);
			}
	...
	for (uint i = 0; i < vecLen; i++) {
		if (a1Chk[i] != a1[i]) {
			fprintf(stderr, "ERROR - A1[%d] = %lf (expected %lf)\n", i, a1[i], a1Chk[i]);
			errCnt++;
		}
		if (a2Chk[i] != a2[i]) {
			fprintf(stderr, "ERROR - A2[%d] = %lf (expected %lf)\n", i, a2[i], a2Chk[i]);
			errCnt++;
		}
		if (a3Chk[i] != a3[i]) {
			fprintf(stderr, "ERROR - A3[%d] = %lf (expected %lf)\n", i, a3[i], a3Chk[i]);
			errCnt++;
		}
	}
	printf("\n");
