...
int main(int argc, char **argv)
{
	...
	printf("Strided Microbenchmark (long int)\n");
	printf("  Initializing arrays\n");
	fflush(stdout);

	x = (uint64_t *)malloc((M1*N) * sizeof(uint64_t));
	y = (uint64_t *)malloc((M1*N) * sizeof(uint64_t));
	...
	uint64_t *cp_x = (uint64_t*)pHtHif->MemAlloc((M1*N) * sizeof(uint64_t));
	uint64_t *cp_y = (uint64_t*)pHtHif->MemAlloc((M1*N) * sizeof(uint64_t));
	if (!cp_x || !cp_y) {
		fprintf(stderr, "ht_cp_malloc() failed.\n");
		exit(-1);
	}
	pHtHif->MemCpy(cp_x, x, (M1*N) * sizeof(uint64_t));
	pHtHif->MemCpy(cp_y, y, (M1*N) * sizeof(uint64_t));
