...
CPersCalc4::PersCalc4()
{
	...
	if (PR1_htValid) {
		...
		case CALC4_STA: {
			...
			uint64_t result = (uint64_t)(SR_op1.read_mem());
	...
	muladd_wrap(GR_htReset, P1_op2Mem, P1_op3Mem, P1_scalar, PR1_htId, P1_i_vld, o_rdy, o_res, o_htId, o_vld, muladd_prm_state1);
	S_primRdy = o_rdy;
	...
	if (o_vld) {
		...
		S_op1.write_addr(o_htId);
		S_op1.write_mem(o_res);
