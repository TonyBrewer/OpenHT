...
int main(int argc, char **argv)
{
	...
	// Send calls to units
	int threadCnt = unitCnt * VADD_HTID_CNT;
	ArrayIdx_t unitYDimOff = 0;
	ArrayIdx_t unitYDimLen = (yDimLen + threadCnt - 1) / threadCnt;

	printf("Starting personality\n");

	for (int thread = 0; thread < threadCnt; thread++) {
		int unit = thread % unitCnt;
	...
	for (int thread = 0; thread < threadCnt; thread++) {
		int unit = thread % unitCnt;
		while (!pAuUnits[unit]->RecvReturn_htmain(threadSum))
			usleep(1000);
		actualSum += threadSum;
	}
