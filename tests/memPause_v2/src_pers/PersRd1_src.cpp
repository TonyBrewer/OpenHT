	...
	if (PR_htValid) {
		...
		case RD1_READ1:
		{
			...
			if (P_loopCnt == PAUSE_LOOP_CNT || P_err) {
				...
				ReadMem_rd1Mem(P_loopCnt & ((1 << RD1_RD_GRP_ID_W) - 1), memRdAddr, PR_HTID, 0, true);
		...
		case RD1_READ2:
		{
			...
			// Issue read request to memory
			ReadMem_rd1Mem(P_loopCnt & ((1 << RD1_RD_GRP_ID_W)-1), memRdAddr, PR_HTID, 1);
		...
		case RD1_READ3:
		{
			...
			// Issue read request to memory
			ReadMem_rd1Mem(P_loopCnt & ((1 << RD1_RD_GRP_ID_W) - 1), memRdAddr, PR_HTID, 2);
		...
		case RD1_READ4:
		{
			...
			// Issue read request to memory
			ReadMem_rd1Mem(P_loopCnt & ((1 << RD1_RD_GRP_ID_W) - 1), memRdAddr, PR_HTID, 3);
		...
		case RD1_LOOP:
		{
			...
			if (P_pauseLoopCnt == 6) {
				...
				ReadMemPause(P_loopCnt & ((1 << RD1_RD_GRP_ID_W) - 1), RD1_TEST1);
			} else {
				P_pauseLoopCnt += 1;
				HtContinue(RD1_LOOP);
			}
