	...
	if (PR_htValid) {
		...
		case RD2_READ1:
		{
			...
			if (P_loopCnt == PAUSE_LOOP_CNT || P_err) {
				...
				ReadMem_rd2Mem(PR_rdGrpId, memRdAddr, PR_HTID, 0);
		...
		case RD2_READ2:
		{
			...
			// Issue read request to memory
			ReadMem_rd2Mem(PR_rdGrpId, memRdAddr, PR_HTID, 1);
		...
		case RD2_READ3:
		{
			...
			// Issue read request to memory
			ReadMem_rd2Mem(PR_rdGrpId, memRdAddr, PR_HTID, 2);
		...
		case RD2_READ4:
		{
			...
			// Issue read request to memory
			ReadMem_rd2Mem(PR_rdGrpId, memRdAddr, PR_HTID, 3);
		...
		case RD2_LOOP:
		{
			...
			if (P_pauseLoopCnt == 3) {
				...
				ReadMemPause(PR_rdGrpId, RD2_TEST1);
			} else {
				P_pauseLoopCnt += 1;
				HtContinue(RD2_LOOP);
			}
		...
		case RD2_TEST1:
		{
			if (GR_rd2Mem.data != (P_loopCnt & 0xf)) {
				HtAssert(0, 0);
				P_err += 1;
			}
		...
		case RD2_TEST2:
		{
			if (GR_rd2Mem.data != ((P_loopCnt + 1) & 0xf)) {
				HtAssert(0, 0);
				P_err += 1;
			}
		...
		case RD2_TEST3:
		{
			if (GR_rd2Mem.data != ((P_loopCnt + 2) & 0xf)) {
				HtAssert(0, 0);
				P_err += 1;
			}
		...
		case RD2_TEST4:
		{
			if (GR_rd2Mem.data != ((P_loopCnt + 3) & 0xf)) {
				HtAssert(0, 0);
				P_err += 1;
			}
