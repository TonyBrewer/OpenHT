...
void CPersRd1::PersRd1()
{
	...
	if (PR_htValid) {
		...
		case RD1_TEST1:
		{
			if (GR_rd1Mem_data() != (P_loopCnt & 0xf)) {
				HtAssert(0, 0);
				P_err += 1;
			}
		...
		case RD1_TEST2:
		{
			if (GR_rd1Mem_data() != ((P_loopCnt + 1) & 0xf)) {
				HtAssert(0, 0);
				P_err += 1;
			}
		...
		case RD1_TEST3:
		{
			if (GR_rd1Mem_data() != ((P_loopCnt + 2) & 0xf)) {
				HtAssert(0, 0);
				P_err += 1;
			}
		...
		case RD1_TEST4:
		{
			if (GR_rd1Mem_data() != ((P_loopCnt + 3) & 0xf)) {
				HtAssert(0, 0);
				P_err += 1;
			}
