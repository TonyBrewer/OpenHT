...
void CPersTest01::PersTest01() {
	if (PR_htValid) {
		...
		case TEST01_WR: {
			GW_test01_0_0_data[0] = (uint32_t)0x00173496bddf8bc0LL;
		...
		case TEST01_ST0: {
			...
			WriteMem_uint32_t(PR_memAddr + 0, GR_test01_0_0_data[0]);
		...
		case TEST01_LD0: {
			...
			ReadMem_test01_0_0_data(PR_memAddr + 0, 0, 1);
			ReadMemPause(TEST01_CHK);
			break;
		}
		case TEST01_CHK: {
			if ((uint32_t)GR_test01_0_0_data[0] != (uint32_t)0x00173496bddf8bc0LL) {
