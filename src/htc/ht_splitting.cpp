...
#if 0
   // Convey (&jle) Avoid problem with predefined CUDA variables temporarily
...
//    SgFunctionDefinition * func_def = NULL;
    ...
    std::set< SgInitializedName *> restoreVars;
    SgFunctionDeclaration* result = HtOmpSupport::generateFunction(body_block, func_name, all_syms, addressOf_syms, restoreVars, NULL, g_scope, true);
   ...
   // insert dim3 threadsPerBlock(xomp_get_maxThreadsPerBlock()); 
    // Convey -- do not do Cuda stuff for Convey
#if 0
...
;
  ...
  void transOmpTarget(SgNode * node)
  {
    ...
    replaceStatement (target, body, true); 
    attachComment (body, "Translated from #pragma omp target ...");
   // removeStatement(target);
...
public:
    ...
    virtual void visit (SgNode* astNode) {
        ...
        case V_SgOmpDeclareTargetStatement:
            {
                SgOmpDeclareTargetStatement *dts = 
                    isSgOmpDeclareTargetStatement(astNode);
                dts->setCompilerGenerated();
                dts->unsetOutputInCodeGeneration();
                inTarget = true;
                break;
            }
        case V_SgOmpEndDeclareTargetStatement:
            {
                SgOmpEndDeclareTargetStatement *edts = 
                    isSgOmpEndDeclareTargetStatement(astNode);
                edts->setCompilerGenerated();
                edts->unsetOutputInCodeGeneration();
                inTarget = false;
                break;
            }
        default:
