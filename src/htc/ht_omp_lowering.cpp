...
// List of parameter lists to host-callable functions (outlined target
// functions).
static std::vector<SgExprListExp*> target_function_arglists;
static SgSwitchStatement *ctl_switch_statement;
static std::vector<string> target_function_names;
static SgMemberFunctionDeclaration *ctl_call_function = 0;
static SgMemberFunctionDeclaration *ctl_return_function = 0;
static SgFunctionDeclaration *htc_control_function = 0;

// Number of target regions
static unsigned int number_of_target_regions;
...
static void transOmpDistribute_others(
    ...
    Rose_STL_Container<SgOmpClause*> clauses = 
        getClause(target, V_SgOmpDistScheduleClause);

    // the case of with the ordered schedule, but without any schedule policy specified
    // treat it as (static, 0) based on GCC's trasnslation
    SgOmpClause::omp_schedule_kind_enum s_kind;
    SgExpression* orig_chunk_size = NULL;

    ROSE_ASSERT(clauses.size() == 1);
    SgOmpDistScheduleClause* s_clause = isSgOmpDistScheduleClause(clauses[0]);
    ROSE_ASSERT(s_clause);
    s_kind = s_clause->get_kind();
    ROSE_ASSERT(s_kind == SgOmpClause::e_omp_schedule_static);

    orig_chunk_size = s_clause->get_chunk_size();
    // if chunk size is not specified, we should not be in this function
    ROSE_ASSERT(orig_chunk_size);

    // (GOMP_distribute_static_start (orig_lower, orig_upper, adj_stride, orig_chunk, &_p_lower, &_p_upper)) 

    string func_start_name;

    if (isDistributeParallelFor) {
        func_start_name = "XOMP_distribute_parallel_for_static_start";
    } else {
        func_start_name = "XOMP_distribute_static_start";
    }

    // Assembling function call expression's parameters
    // first three are identical for all cases: 
    // we generate inclusive upper (-1) bounds after loop normalization, 
    // gomp runtime calls expect exclusive upper bounds
    ...
    SgExprListExp* para_list_i = 
        buildExprListExp(copyExpression(orig_lower), 
                         copyExpression(orig_upper), 
                         createAdjustedStride(orig_stride, isIncremental),
                         copyExpression(orig_chunk_size));

    string func_init_name = "XOMP_distribute_static_init";

    SgExprStatement* func_init_stmt = 
        buildFunctionCallStmt(func_init_name, buildVoidType(), 
                              para_list_i, bb1);
    ...
    func_start_exp = buildFunctionCallExp(func_start_name, 
                                          buildBoolType(), para_list, bb1);
    ...
    n_exp_list = 
        buildExprListExp(buildAddressOfOp(buildVarRefExp(lower_decl)), 
                         buildAddressOfOp(buildVarRefExp(upper_decl)));
    ...
    // do {} while (GOMP_distribute_static_next (&_p_lower, &_p_upper))
    func_next_exp = buildFunctionCallExp("XOMP_distribute_static_next",
                                        buildBoolType(),
                                         n_exp_list, 
                                         bb1);
    ...
    transOmpVariables(target, bb1, orig_upper);

    string func_loop_end_name = "XOMP_distribute_end_nowait"; 
    SgExprStatement* end_func_stmt = 
        buildFunctionCallStmt(func_loop_end_name, buildVoidType(), NULL, bb1);
    appendStatement(end_func_stmt, bb1);

    return;

}  // transOmpDistribute_others


void transOmpDistribute(SgNode* node, bool isDistributeParallelFor) {

    ROSE_ASSERT(node != NULL);
    ROSE_ASSERT(isSgOmpDistributeStatement(node));
    ...
    SgStatement * body =  target->get_body();
    ROSE_ASSERT(body != NULL);
    // The OpenMP syntax requires that the omp distribute pragma is 
    // immediately followed by the for loop.
    SgForStatement * for_loop = isSgForStatement(body);
    ROSE_ASSERT (for_loop != NULL);

    // Step 1. Loop normalization
    // we reuse the normalization from SageInterface, though it is 
    // different from what gomp expects.
    // The point is to have a consistent loop form. We can adjust 
    ...
    // step 2. Insert a basic block to replace SgOmpDistributeStatement
    // This newly introduced scope is used to hold loop variables, 
...
#if 1 // CONVEY-added change.
    // Use original ivar type, as we will emit explicit code for the 
    // xomp calls later in LateOmpLowering.  We want to compute iteration
    // spaces with the least width datatype possible for area optimization.
    loop_var_type = orig_index->get_type();
#endif
    ...
    index_decl = 
        buildVariableDeclaration("p_dist_index_", loop_var_type , NULL,bb1);
    lower_decl = 
        buildVariableDeclaration("__htc_t_p_dist_lower_", loop_var_type , NULL,bb1);
    upper_decl = 
        buildVariableDeclaration("p_dist_upper_", loop_var_type , NULL,bb1); 
    ...
    if (clauses.size()) {
        ...
        // only one dist_schedule clause is permitted
        ROSE_ASSERT(clauses.size() == 1);

        SgOmpDistScheduleClause* s_clause = 
            isSgOmpDistScheduleClause(clauses[0]);
        ROSE_ASSERT(s_clause);
    ...
    //  step 3. Translation for omp distribute
    // if chunk size is specified, use long form
    if (my_chunk_size != NULL) {
       return transOmpDistribute_others( target, index_decl, lower_decl, 
                                         upper_decl, bb1, 
                                         isDistributeParallelFor);
    }

    //void XOMP_distribute_default(int lower, int upper, int stride, long *n_lower, long * n_upper)
    ...
    SgStatement * call_stmt =  
        buildFunctionCallStmt ("XOMP_distribute_default", 
                               buildVoidType(), 
                               call_parameters, 
                               bb1);
...
#endif    
    if (hasClause(target, V_SgOmpNowaitClause) || distribute) 
    {
  ...
  */
  ...
  //void transOmpFor(SgNode* node)
  {
    ...
    SgOmpDistributeStatement* distribute_statement =
        isSgOmpDistributeStatement(body);
    if (distribute_statement) {
        body = distribute_statement->get_body();
    }
    ...
    // Analyze whether long or short form is needed
    bool hasOrder = false;
    if (hasClause(target, V_SgOmpOrderedClause))
      hasOrder = true;
    ...
    bool long_form = !useStaticSchedule(target) || 
        hasOrder || 
        hasSpecifiedSize;

    // process distribute first
    if (distribute_statement) {
        transOmpDistribute(distribute_statement, true);
    }
    ...
    if (distribute_statement) {
        replaceStatement(loop, bb1, true);
        replaceStatement(target,isSgOmpForStatement(node)->get_body(), true);
    } else {
        replaceStatement(target, bb1, true);
    }
   ...
   {  
...
#if 1 // CONVEY-added
    lower_decl = buildVariableDeclaration("__htc_t_p_lower_", loop_var_type , NULL,bb1); 
#else
    ...
    if (long_form) 
    {
        transOmpLoop_others( target, index_decl, lower_decl, upper_decl, bb1, loop, distribute_statement != NULL);
    }
    else 
    {
      ...
      SgExprListExp* call_parameters = 
          buildExprListExp(copyExpression(orig_lower), 
                           ...
                           e4, e5);
      SgStatement * call_stmt =  
          buildFunctionCallStmt ("XOMP_loop_default", buildVoidType(), 
                                 call_parameters, bb1);
      appendStatement(call_stmt, bb1);
      ...
      if (!hasClause(target, V_SgOmpNowaitClause) && !distribute_statement) 
      {
  ...
  void transOmpTargetLoop(SgNode* node)
{
  ...
  ROSE_ASSERT(isSgOmpForStatement(node) ||
              isSgOmpDoStatement(node) ||
              isSgOmpDistributeStatement(node));
...
//SgFunctionDeclaration* generateOutlinedTask(SgNode* node, std::string& wrapper_name, ASTtools::VarSymSet_t& syms, std::set<SgInitializedName*>& readOnlyVars, ASTtools::VarSymSet_t&pdSyms3)
{
  ...
  // must be either omp task or omp parallel or omp distribute parallel for
  ROSE_ASSERT(isSgOmpTaskStatement(node) ||
              isSgOmpParallelStatement(node) ||
              isSgOmpDistributeParallelForStatement(node));
...
//    target->get_endOfConstruct()->display();
    ...
    { 
      ...
      if (syms.size()==0 || 1)  /// Convey changed this
                                    ...
                                    std::set<SgSymbol*>& atom_syms) // variable symbols which are non-aggregate types: scalar, pointer, etc
  {
   // categorize the variables:
    for (SgInitializedNamePtrList::const_iterator iter = all_vars.begin(); iter != all_vars.end(); iter ++)
    {
      ...
      SgType * type = sym->get_type()->stripTypedefsAndModifiers();
      ...
      {
          cerr<<"Error. categorizeMapClauseVariables() of omp_lowering.cpp: unhandled map clause variable type:"<<type->class_name()<< endl;
      }
    }
   ...
   // this will avoid name collisions when there are multiple "omp target" within one big scope
    ...
    if (device_clauses.size()) {
        ...
        for (iter = device_clauses.begin(); iter != device_clauses.end(); 
            ...
            std::cerr << "device clause is " << dev->unparseToString() << std::endl;
  ...
  for (std::set<SgSymbol*>::const_iterator iter = array_syms.begin(); iter != array_syms.end(); iter ++)
   {
    ...
    // I believe that all array variables need allocations on GPUs, regardless their map operations (alloc, in, out, or inout)
     ...
     // Convey -- we just want to use the original variable

     string dev_var_name = "_dev_" + to_string<int>(function_selector) + 
         "_" + orig_name; 
    ...
    // For "omp target data", 
     ...
     if (need_generate_data_stmt) // Only if we are in the mode of inserting data handling statements
     {
       SgExprStatement* mem_alloc_stmt = buildAssignStatement(buildVarRefExp(dev_var_name, insertion_scope), 
           buildCastExp ( buildFunctionCallExp(SgName("__htc_cp_alloc"), 
     ...
     if (need_generate_data_stmt)
     {
           ...
           ((map_inout_clause) && (isInClauseVariableList (map_inout_clause,sym))) )
       {
         ...
         SgExprStatement* mem_copy_to_stmt = buildFunctionCallStmt (SgName("__htc_memcpy"), 
             buildPointerType(buildVoidType()),
             parameters,
             insertion_scope);
     ...
     // Convey -- reverse steps 7 and 6 so when we insert the statements
     // frees will end up last.
     ...
     if (need_generate_data_stmt)
     {
       SgExprStatement* mem_dealloc_stmt = 
         buildFunctionCallStmt(SgName("__htc_cp_free"),
             buildVoidType(),
       ...
       insertStatementAfter(insertion_anchor_stmt, mem_dealloc_stmt);
     }
     ...
     if (need_generate_data_stmt)
     {
           ...
           ( (map_inout_clause) && (isInClauseVariableList (map_inout_clause,sym))))
       {
         ...
         SgExprStatement* mem_copy_back_stmt = buildFunctionCallStmt (SgName("__htc_memcpy"), 
             buildPointerType(buildVoidType()),
             parameters, 
             insertion_scope);
         // appendStatement(mem_copy_back_stmt, insertion_anchor_stmt->get_scope()); 
         insertStatementAfter(insertion_anchor_stmt, mem_copy_back_stmt);
...
SgClassDeclaration* declareCHtHif(SgGlobal *glob_scope) {
    ...
    htdecl->unsetOutputInCodeGeneration();
                           ...
                           SgGlobal *glob_scope) 
{
    ...
    units->unsetOutputInCodeGeneration();
    return units;
}
                           ...
                           SgScopeStatement *scope) 
{
    ...
    SgVariableDeclaration *unit_index = 
        buildVariableDeclaration("__htc_unit_index", type, 0, scope);
    insertStatementAfter(htdecl, unit_index);

    std::string before_text = "\n#ifdef CNY_HTC_HOST\n";
    before_text += "#include \"hif.h\"\n";
    before_text += "#endif /* CNY_HTC_HOST */\n";

    SageInterface::addTextForUnparser(unit_index,
                                      before_text,
...
SgForStatement *
addUnitCountForLoopTextAroundStmt(SgStatement *S,
                                  SgVariableDeclaration *unit_index,
                                  SgExpression *num_teams) {

    SgStatement* init_stmt = 
        buildAssignStatement(buildVarRefExp(unit_index), buildIntVal(0));

    SgStatement* test_stmt = 
        buildExprStatement(buildLessThanOp(buildVarRefExp(unit_index),
                                           deepCopy(num_teams)));
    SgExpression* incr_exp = 
        buildPlusPlusOp(buildVarRefExp(unit_index),SgUnaryOp::postfix);

    SgForStatement* for_stmt = 
        buildForStatement(init_stmt, test_stmt, incr_exp, S);

    return(for_stmt);
}

SgExpression* transOmpTeams(SgNode *node)
{

    SgExpression *num_teams_expr = NULL;

    // Sanity check first
    ROSE_ASSERT(node != NULL);
    SgOmpTeamsStatement* teams = isSgOmpTeamsStatement(node);
    ROSE_ASSERT (teams != NULL);
    SgOmpTeamsStatement* teams_directive_stmt = teams;

    // Check that parent is a Target Directive
    SgNode* parent = teams->get_parent();
    ROSE_ASSERT(parent != NULL);
    if (isSgBasicBlock(parent)) // skip the padding block in between.
        parent= parent->get_parent();
    if (!isSgOmpTargetStatement(parent)) {
        std::cerr << 
            "Teams pragma is not enclosed by a target pragma.\n" <<
            "The teams pragma will be ignored\n";
        std::cerr << "The parent is a " << parent->sage_class_name() << std::endl;
        return NULL;
    }

    // Handle num_teams clause
    Rose_STL_Container<SgOmpClause*> num_teams_clauses; 
    num_teams_clauses = getClause(teams_directive_stmt, V_SgOmpNumTeamsClause);

    if (num_teams_clauses.size() > 1) {
        cerr<<"Warning: found " << num_teams_clauses.size() << " num_teams clauses.  Only the first one will be used.  The others are ignored.\n";
    }
    if (num_teams_clauses.size()) {
        SgOmpNumTeamsClause * t_cls = 
            isSgOmpNumTeamsClause(num_teams_clauses.at(0));
        num_teams_expr = t_cls->get_expression();
    }

    // replace the teams pragma with the its body
    SgStatement* body = teams_directive_stmt->get_body();
    ROSE_ASSERT(body != NULL );
    body->set_parent(NULL);
    teams_directive_stmt->set_body(NULL);

    replaceStatement(teams_directive_stmt, body);

    return num_teams_expr;
}

// Adapted from Outliner::appendIndividualFunctionCallArgs
// Casts all arguments to UnsignedLongLong.
void appendIndividualFunctionCallArgsWithCastToULL (
         ...
         SgExprListExp* e_list)
{
      ...
      i != syms.end (); ++i)
  {
    ...
    ROSE_ASSERT (i_arg);
    e_list->append_expression(buildCastExp(i_arg, 
...
int maxControlArgs() {
    ...
    for (i = 0; i < target_function_arglists.size(); i++) {
        SgExprListExp *el = target_function_arglists.at(i);
        int el_size = el->get_expressions().size();
        if (el_size > result) {
            result = el_size;
        }
    }
...
void finishControlFunction(SgFunctionDeclaration *func) {
    int i, j;
    SgFunctionParameterList* params = func->get_parameterList ();
    ROSE_ASSERT (params);
    SgFunctionDefinition* fdef = func->get_definition ();
    ROSE_ASSERT (fdef);
    SgBasicBlock* body = fdef->get_body ();
    ROSE_ASSERT (body);



    int maxArgs = maxControlArgs();

    // Add "generic" arguments to parameter list.   Skip first 5, which
    // end in the function_selector.
    for (i = 5; i < maxArgs; i++) {
        std::string name("__arg");
        name += to_string<int>(i-5);
        SgInitializedName *iname = 
            buildInitializedName(name, buildUnsignedLongLongType());
        appendArg(params, iname);
    }

    // Add padding arguments onto calls from the host
    for (i = 0; i < target_function_names.size(); i++) {
        SgExprListExp* exp_list_exp = SageBuilder::buildExprListExp();

        SgExprListExp* el = target_function_arglists.at(i);
        SgExpressionPtrList epl = el->get_expressions();
        int el_size = epl.size();

        for (j = el_size; j < maxArgs; j++) {
            appendExpression(el, buildIntVal(0));
        }
    }
...
void addCallToTargetFunction(SgFunctionDeclaration *func,
                             string &func_name,
                             SgExpressionPtrList epl,
                             int case_num) {
    static int maxArgs = -1;
    ...
    SgBasicBlock *switchBody = 
        isSgBasicBlock(ctl_switch_statement->get_body());
    ...
    for (j = 0; j < el_size; j++) {
        SgExpression *arg = epl.at(j);
        SgType *type = arg->get_type();
        std::string name("__arg");
        name += to_string<int>(j);

        // Create variable in function if not already there
        if (j > maxArgs) {
            maxArgs = j;
            SgInitializedName *iname = 
                buildInitializedName(name, buildUnsignedLongLongType());
            iname->set_scope(fdef);
            fdef->insert_symbol(iname->get_name(), 
                                new SgVariableSymbol(iname));
        }

        appendExpression(exp_list_exp,
                         buildCastExp(buildVarRefExp(name, fdef), type));
    }

    SgExprStatement *callStmt =
        buildFunctionCallStmt(func_name,
                              buildVoidType(),
                              exp_list_exp, 
                              fdef);
    SgCaseOptionStmt *caseStmt = 
        buildCaseOptionStmt(buildIntVal(case_num),
                            buildBasicBlock(callStmt,
                                            buildBreakStmt()));
    appendStatement(caseStmt, switchBody);

}  // addCallToTargetFunction


SgFunctionDeclaration *createControlFunction(SgScopeStatement *scope) {

    string func_name = "__HTC_HOST_ENTRY_CTL";
    SgFunctionParameterList *parameterList = buildFunctionParameterList();

    appendArg(parameterList,
              buildInitializedName("function_selector", buildUnsignedIntType()));
    ...
    SgBasicBlock *result_body = result->get_definition()->get_body();
    SgFunctionDefinition* fdef = result->get_definition ();
    ROSE_ASSERT (fdef);

    SgBasicBlock *switchBody = buildBasicBlock();
    SgExpression *switchSelect = buildVarRefExp("function_selector", fdef);
    ctl_switch_statement = buildSwitchStatement(switchSelect, switchBody);
    appendStatement(ctl_switch_statement, result_body);
    ...
    SageInterface::appendStatement(result, scope);
...
void transOmpTarget(SgNode * node, SgExpression * num_teams)
  {
    // Sanity check first
    ROSE_ASSERT(node != NULL);
    ...
    if (first) {
        ...
        if (number_of_target_regions > 1) {
            htc_control_function = 
                createControlFunction(getGlobalScope(target));
        }
    ...
    unsigned int function_selector = target_function_arglists.size();

    transOmpMapVariables (target_directive_stmt, 0, body_block, 
                          all_syms, addressOf_syms, function_selector);
    ...
    string func_name = Outliner::generateFuncName(target);
    string host_call_func_name;

    // Convey -- prefix host entry function name
    if (number_of_target_regions > 1) {
        func_name = "__HTC_ENTRY_" + func_name;
        target_function_names.push_back(func_name);
        host_call_func_name = "__HTC_HOST_ENTRY_CTL";
    } else {
        func_name = "__HTC_HOST_ENTRY_" + func_name;
        host_call_func_name = func_name;
    }
    ...
    SageInterface::cutPreprocessingInfo(result,
    ...
    if (save_buf.size()) {
        SgVariableDeclaration *dummy = 
            SageBuilder::buildVariableDeclaration(
                ...
                SageBuilder::buildIntType(), 0, result->get_scope());
        SageInterface::setExtern(dummy);

        SageInterface::insertStatementBefore(result, dummy);
    ...
    if (number_of_target_regions > 1) {
        ...
        target_function_arglists.push_back(exp_list_exp);
        // Everything cast to ULL on calls from host
        appendIndividualFunctionCallArgsWithCastToULL (all_syms, 
                                                       varsUsingOriginalForm, 
                                                       exp_list_exp);

        // Create call from CTL function
        SgExprListExp* exp_list_exp2 = SageBuilder::buildExprListExp();
        Outliner::appendIndividualFunctionCallArgs (all_syms, 
                                                    varsUsingOriginalForm, 
                                                    exp_list_exp2);

        addCallToTargetFunction(htc_control_function,
                                func_name,
                                exp_list_exp2->get_expressions(),
                                function_selector);
    } else {
        Outliner::appendIndividualFunctionCallArgs (all_syms, 
                                                    varsUsingOriginalForm, 
    ...
    SgExpression *unit_index_expr;

#if 0
    // RANDY:  Handle device clause
    Rose_STL_Container<SgOmpClause*> device_clauses; 
    device_clauses = getClause(target_directive_stmt, V_SgOmpDeviceClause);

    if (device_clauses.size() > 1) {
        cerr<<"Warning: found " << device_clauses.size() << " device clauses.  Only the first one will be used.   The others are ignored.\n";
    }
    if (device_clauses.size()) {
        SgOmpDeviceClause * d_cls = isSgOmpDeviceClause(device_clauses.at(0));
        unit_index_expr = d_cls->get_expression();
    } else {
        unit_index_expr = buildIntVal(0);
    }

    SgInitializer *init = buildAssignInitializer(deepCopy(unit_index_expr));

    unit_index = buildVariableDeclaration("__htc_unit_index", 
                                          buildIntType(), 
                                          init,
                                          p_scope);

    insertStatementBefore(target, unit_index);
#endif
    ...
    if (ctl_call_function == NULL) {
        call_function = SageBuilder::buildDefiningMemberFunctionDeclaration (
            "SendCall_" + host_call_func_name, 
            buildVoidType(), 
            buildFunctionParameterList_nfi(),
            htdecl->get_definition());
        htdecl->get_definition()->append_member(call_function);
        ctl_call_function = call_function;
    } else {
        call_function = ctl_call_function;
    }
    ...
    if (ctl_return_function == NULL) {
        return_function = SageBuilder::buildDefiningMemberFunctionDeclaration (
            "RecvReturn_" + host_call_func_name, 
            buildIntType(), 
            buildFunctionParameterList_nfi(),
            htdecl->get_definition());
        htdecl->get_definition()->append_member(return_function);
        ctl_return_function = return_function;
    } else {
        return_function = ctl_return_function;
    }
    ...
    appendExpression(usleep_args, buildIntVal(0));
...
public:
    ProcessDeclareTarget(SgSourceFile *f) : inTarget(false), file(f) {} 

    virtual void visit (SgNode* astNode) {

        // return if node is not in the main file
        if (!astNode->get_file_info()->isSameFile(file)) {
            return;
        }
...
void handleDeclareTarget(SgSourceFile *file) {
    ProcessDeclareTarget pdt(file);;
    pdt.traverseWithinFile(file, preorder, STRICT_INPUT_FILE_TRAVERSAL);
}

void hideGlobalsFromCoproc(SgSourceFile *file) {
    Rose_STL_Container< SgNode * > sgVarDecls;
    sgVarDecls = NodeQuery::querySubTree (file, V_SgVariableDeclaration);
    ...
    for (Rose_STL_Container<SgNode*>::iterator iter = sgVarDecls.begin();
        ...
        if (isSgGlobal(initName->get_scope())) {
            // std::cerr << "global variable " << initName->get_name().getString() << std::endl;
            if (! isScalarType(initName->get_type())) {
                // Only keep global variable declarations for host
                std::string before_text = "\n#ifdef CNY_HTC_HOST\n";
                SageInterface::addTextForUnparser(varDecl,
                                                  before_text,
                                                  AstUnparseAttribute::e_before);
                // Dummy call with empty string so the e_after on the next
                // call will be honored.
                ...
                std::string after_text =
                    "\n#endif /* CNY_HTC_HOST */\n";
                SageInterface::addTextForUnparser(varDecl, after_text,
                                                  AstUnparseAttribute::e_after);
...
void htc_split(SgSourceFile* file)
{
    ...
    Rose_STL_Container<SgNode*> targetList = 
        NodeQuery::querySubTree(file, V_SgOmpTargetStatement);
    number_of_target_regions = targetList.size();

    target_function_arglists.clear();
    target_function_names.clear();
    ...
    for ( ;nodeListIterator !=nodeList.rend();  ++nodeListIterator) {
        ...
        case V_SgOmpTargetStatement:
            {
                transOmpTarget(node, num_teams);
                num_teams = NULL;
                break;
            }
        case V_SgOmpTeamsStatement:
            {
                num_teams = transOmpTeams(node);
                break;
            }
    ...
    if (number_of_target_regions > 1) { 
        finishControlFunction(htc_control_function);
    }

    hideGlobalsFromCoproc(file);
...
#if 0
  // Convey ifdef'ed this out for HTC
  if (!SageInterface::is_Fortran_language() )
...
//    cout<<"Debug lower_omp(). stmt:"<<node<<" "<<node->class_name() <<" "<< node->get_file_info()->get_line()<<endl;
...
#if 0
    std::cerr << "Processing a node of kind " <<
    ...
    switch (node->variantT())
    {
...
#if 0
              SgOmpClauseBodyStatement* newtarget;
              newtarget = transOmpDistribute(node);
              transOmpLoop(newtarget);
#endif
              //    transOmpParallel(node);
          }
          break;
      ...
      case V_SgOmpDistributeStatement:
          {
              SgNode *parent = node->get_parent();
              if (parent && (parent->variantT() == V_SgOmpForStatement)) {
                  //Defer processing to OmpFor\n";
              } else {
                  transOmpDistribute(node, false);
              }
          }
          break;
      case V_SgOmpForStatement:
