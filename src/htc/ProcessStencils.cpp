...
static void buildStencilCode(fceInfo &fi, SgFunctionDeclaration *newFD)
{
  ...
  if (!fi.useStreaming) {
    ...
    "			// two cycles for timing. OFF = ((Y_ORIGIN * (PR1_cols + X_SIZE-1) + X_ORIGIN) * sizeof(StType_t);\n"
    "			//\n"
    "			uint32_t offset = " + yorgStr + " * (PR1_cols + " + xsizeStr+ "-1);\n"
    "			S_rdAddr = PR1_rdAddr;\n"
    "			S_wrAddr = offset;\n"
    "			S_rdRowIdx = 0;\n"
    "			S_wrRowIdx = " + yorgStr + ";\n"
    ...
    "			S_bStart = true;\n"
    "\n"
    "			S_wrAddr = ((uint32_t)S_wrAddr + " + xorgStr + ") * sizeof(StType_t) + PR1_wrAddr;\n"
    "\n"
    "			StencilBufferInit_" + stbName + "((ht_uint11)PR1_cols, (ht_uint11)PR1_rows);\n"
    "\n"
