...
CPreProcess::CPreProcess()
{
	...
	m_defineTbl.Insert("_HTV");
	m_defineTbl.Insert("__LINE__");
	...
	for (;;) {  // loop until non-directive line found
		...
		if (!GetLineWithCommentsStripped(lineBuf)) {
			...
			if (m_pFs->m_bSkipStack.size() != 1) {
				ParseMsg(PARSE_ERROR, "EOF - Expected #endif");
		...
		if (*pPos++ != '#') {
			if (m_bSkipLines)
				...
				if (m_pWriteFp)
					fprintf(m_pWriteFp, "%s(%d) %s\n", m_pFs->m_lineInfo.m_fileName.c_str(), m_pFs->m_lineInfo.m_lineNum, lineBuf.c_str());
		...
		if (!m_bSkipLines) {
			...
			case dir_else:
				if (m_pFs->m_bSkipStack.size() <= 1 ||
					m_pFs->m_bElseStack.back() == true) {
						ParseMsg(PARSE_ERROR, "Unexpected #else directive");
						break;
				}
				m_bSkipLines = true;
			...
			case dir_elif:
				if (m_pFs->m_bSkipStack.size() <= 1 ||
					m_pFs->m_bElseStack.back() == true) {
						ParseMsg(PARSE_ERROR, "Unexpected #elif directive");
						break;
				}
				m_bSkipLines = true;
				m_pFs->m_bSkipStack.back() = true;
			...
			case dir_endif:
				if (m_pFs->m_bSkipStack.size() <= 1) {
					ParseMsg(PARSE_ERROR, "Unexpected #endif directive");
					break;
				}
				m_pFs->m_bSkipStack.pop_back();
				m_pFs->m_bElseStack.pop_back();
			...
			case dir_pragma:
				...
				break;  // Ignore
			default:
				ParseMsg(PARSE_ERROR, "Unknown preprocessing directive");
				return true;
			}
		} else {
			switch (m_directiveTbl.Find(ParseIdentifier(pPos))) {
...
void CPreProcess::PerformMacroExpansion(string &lineBuf)
{
	...
	vector<string> prevExpansionList;
	int prevStartPos = -1;
	...
	while (*pPos != '\0') {
		...
		if (!(pMacro = m_defineTbl.Find(identifier)))
			continue;
		...
		if (pMacro->IsParenReqd()) {
			...
			int argId = 0;
			for (;;) {
				bool bVariadicArg = pMacro->IsVariadicArg(argId);

				while (*pPos == ' ' || *pPos == '\t')
					pPos += 1;
				const char *pStartArgPos = pPos;
				int parenLvl = 0;
				while (parenLvl > 0 || ((bVariadicArg ||*pPos != ',') && *pPos != ')')) {
					if (*pPos == '\0') {
				...
				while (pPos > pStartArgPos && (*(pPos-1) == ' ' || *pPos == '\t'))
					pPos -= 1;
				if (*pPos == '\0') {
					ParseMsg(PARSE_ERROR, "Macro argument error");
					return;
				}
		...
		// check for macro expansion recursion
		if (startPos != prevStartPos) {
			prevExpansionList.clear();
			prevStartPos = startPos;
		} else {
			for (size_t i = 0; i < prevExpansionList.size(); i += 1) {
				if (prevExpansionList[i] == expansion) {
					ParseMsg(PARSE_ERROR, "Macro expansion recursion detected");
					for (size_t j = 0; j < prevExpansionList.size(); j += 1)
						ParseMsg(PARSE_INFO, "%s", prevExpansionList[j].c_str());
					ParseMsg(PARSE_FATAL, "");
				}
			}
			prevExpansionList.push_back(expansion);
		}
...
void CPreProcess::ExpandMacro(CDefine *pMacro, vector<string> &argList, string &expansion)
{
	// find identifiers in line and check for macro names
	...
	while (*pPos != '\0') {
		...
		if (!(pParam = pMacro->FindParam(identifier)))
			continue;
		...
		expansion.replace(startPos, endPos-startPos,
			argList[pParam->GetParamId()]);

		pPos = expansion.c_str() + startPos + argList[pParam->GetParamId()].size();
	}
}
...
void CPreProcess::PreProcessDefine(string &lineBuf, const char *&pPos)
{
	...
	if (defineName.length() == 0) {
		ParseMsg(PARSE_ERROR, "Missing define name");
		return;
	}
	CDefine *pDefine = m_defineTbl.Insert(defineName);

	if (!pDefine->IsNew()) {
		ParseMsg(PARSE_WARNING, CPreProcess::GetFileLineInfo(), "Redefinition of %s", defineName.c_str());
		ParseMsg(PARSE_INFO, pDefine->GetLineInfo(), "     Original definition");
	}
	...
	if (*pPos == '(') {
		...
		for (int paramId = 0;;) {
			...
			if (strncmp(pPos, "...", 3) == 0) {
				...
				SkipSpace(pPos);
				if (*pPos != ')') {
					ParseMsg(PARSE_ERROR, "variadic macro argument must be last argument");
					return;
				}
			} else
				paramName = ParseIdentifier(pPos);

			if (paramName.length() == 0) {
				ParseMsg(PARSE_ERROR, "Parameter name error");
				return;
			}
			if (!pDefine->InsertParam(paramName, paramId++)) {
				ParseMsg(PARSE_ERROR, "Macro has redundant parameter name '%s'", paramName.c_str());
				return;
			}
			SkipSpace(pPos);
		...
		if (*pPos++ != ')') {
			ParseMsg(PARSE_ERROR, "Parameter list error");
			return;
		}
	}
	...
	pDefine->SetLineInfo(CPreProcess::GetFileLineInfo());
}
...
void CPreProcess::PreProcessUndef(string &lineBuf, const char *&pPos)
{
	...
	if (undefName.length() == 0) {
		ParseMsg(PARSE_ERROR, "Missing undef name");
		return;
	}
	m_defineTbl.Remove(undefName);
}
...
void CPreProcess::PreProcessInclude(string &lineBuf, const char *&pPos)
{
	...
	if (*pPos != '"') {
		ParseMsg(PARSE_ERROR, "Include file error");
		return;
	}
	...
	for (size_t i = 0; i < g_pHtfeArgs->GetIncludeDirs().size(); i += 1) {
		string pathName = g_pHtfeArgs->GetIncludeDirs()[i] + fileName;
	...
	if (!Open(fileName, true))
		ParseMsg(PARSE_ERROR, "Could not open file '%s'", fileName.c_str());
}
...
bool CPreProcess::PreProcessIf(string &lineBuf, const char *&pPos)
{
	...
	if (identifier.length() == 0) {
		ParseMsg(PARSE_ERROR, "Unknown #if directive format");
		return false;
	}
	...
	if (identifier == "defined") {
		...
		return (m_defineTbl.Find(identifier) == 0) != bNot;
	} else {
		SkipSpace(pPos);
...
bool CPreProcess::ParseExpression(const char *&pPos, int &rtnValue)
{
	SkipSpace(pPos);
	...
	for (;;) {
		...
		if (tk == tk_num_int || tk == tk_num_hex) {
			...
			if (GetTokenString() == "defined") {
				...
				operandStack.push_back(m_defineTbl.Find(GetTokenString()) == 0 ? 0 : 1);

			} else {
				CDefine *pMacro;
				if (!(pMacro = m_defineTbl.Find(GetTokenString())))
					...
					//	ParseMsg(PARSE_ERROR, "could not convert macro name to value '%s='%s'\n",
		...
		case tk_greaterGreater:
			...
			break;
		default:
			ParseMsg(PARSE_ERROR, "Unknown operator type");
			return false;
		}
	}
}

void CPreProcess::EvaluateExpression(EToken2 tk, vector<int> &operandStack,
	vector<int> &operatorStack)
{
	int op1, op2, rslt;
...
int CPreProcess::GetTokenPrec(EToken2 tk)
{
	switch (tk) {
	case tk_unaryPlus:
	case tk_unaryMinus:
...
CPreProcess::EToken2 CPreProcess::GetNextToken(const char *&pPos) 
{
	const char *pInitPos;
	...
	default:
		...
		ParseMsg(PARSE_ERROR, "unknown preprocess token type");
		return tk_error;
	}
}
	...
	if (m_tk == tk_num_int) {
		while (*pStr) {
			if (value >= 0xffffffff / 10) {
				ParseMsg(PARSE_ERROR, "integer too large, (%s)", GetTokenString().c_str());
				return 0;
			}
...
bool CPreProcess::PreProcessIfdef(string &lineBuf, const char *&pPos)
{
	// pPos points just after directive
	SkipSpace(pPos);
	...
	string identifier = ParseIdentifier(pPos);
	if (identifier.length() == 0) {
		ParseMsg(PARSE_ERROR, "Unknown #ifdef directive format");
		return false;
	}
	...
	return m_defineTbl.Find(identifier) == 0; // skip lines if true
}

bool CPreProcess::PreProcessIfndef(string &lineBuf, const char *&pPos)
{
	// pPos points just after directive
	SkipSpace(pPos);
	...
	string identifier = ParseIdentifier(pPos);
	if (identifier.length() == 0) {
		ParseMsg(PARSE_ERROR, "Unknown #ifmdef directive format");
		return false;
	}
	...
	do {
		for (;;) {
			if (!GetLineFromFile(lineBuf, bContinue || m_bInComment)) {
				if (bContinue)
					ParseMsg(PARSE_ERROR, "File ended with continuation");
				else
...
bool CPreProcess::GetLineFromFile(string &lineBuf, bool bAppendLine)
{
	...
	while (pPos == pEnd || *pPos != '\n') {
		if (pPos == pEnd) {
			...
			if (rdBytes < 0) {
				ParseMsg(PARSE_ERROR, "File read error");
				return false;
			}
...
void CPreProcess::ParseMsg(EErrorMsgType msgType, const char *msgStr, ...)
{
	va_list marker;
	va_start( marker, msgStr );
...
void CPreProcess::ParseMsg(EErrorMsgType msgType, const CLineInfo &lineInfo, const char *msgStr, ...)
{
	va_list marker;
	va_start( marker, msgStr );
