...
void CDsnInfo::GenModOhmStatements(CInstance * pModInst)
{
	...
	CHtCode	& ohmPreInstr = pMod->m_clkRate == eClk1x ? m_ohmPreInstr1x : m_ohmPreInstr2x;
	CHtCode	& ohmPostInstr = pMod->m_clkRate == eClk1x ? m_ohmPostInstr1x : m_ohmPostInstr2x;
	CHtCode	& ohmReg = pMod->m_clkRate == eClk1x ? m_ohmReg1x : m_ohmReg2x;
	CHtCode	& ohmPostReg = pMod->m_clkRate == eClk1x ? m_ohmPostReg1x : m_ohmPostReg2x;
	...
	ohmReg.Append("\tr_%sToHti_ohmAvlCnt = %s ? (ht_uint1)1 : c_%sToHti_ohmAvlCnt;\n",
		pMod->m_modName.Lc().c_str(), reset.c_str(), pMod->m_modName.Lc().c_str());
	ohmReg.Append("\n");
	...
	ohmPostReg.Append("\tc_bSendHostMsgBusy = r_%sToHti_ohmAvlCnt == 0 || %s_RND_RETRY && ((g_rndRetry() & 1) == 1);\n",
		pMod->m_modName.Lc().c_str(), pModInst->m_instName.Upper().c_str());
