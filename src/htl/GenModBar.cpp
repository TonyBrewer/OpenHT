...
void CDsnInfo::InitAndValidateModBar()
{
	...
	for (size_t modIdx = 0; modIdx < m_modList.size(); modIdx += 1) {
		...
		pMod->m_rsmSrcCnt += pMod->m_barrierList.size();
...
void CDsnInfo::GenModBarStatements(CModInst * pModInst)
{
	...
	int rlsCntBits = 0;
	switch (pMod->m_instSet.GetReplCnt(pModInst->m_instId)) {
	...
	for (size_t barIdx = 0; barIdx < pMod->m_barrierList.size(); barIdx += 1) {
		...
		bool bMimicHtCont = pMod->m_threads.m_htIdW.AsInt() == 0 && pMod->m_instSet.GetReplCnt(pModInst->m_instId) == 1;

		CHtCode & barPreInstr = pMod->m_clkRate == eClk2x ? m_barPreInstr2x : m_barPreInstr1x;
		CHtCode & barPostInstr = pMod->m_clkRate == eClk2x ? m_barPostInstr2x : m_barPostInstr1x;
		CHtCode & barReg = pMod->m_clkRate == eClk2x ? m_barReg2x : m_barReg1x;
		CHtCode & barOut = pMod->m_clkRate == eClk2x ? m_barOut2x : m_barOut1x;
		...
		m_barFuncDef.Append("void CPers%s%s::HtBarrier%s(",
			unitNameUc.c_str(), pModInst->m_instName.Uc().c_str(), barName.c_str());
		...
		g_appArgs.GetDsnRpt().AddText("sc_uint<%s_INSTR_W> releaseInstr, sc_uint<%s_HTID_W+%d> releaseCnt)\n",
			pMod->m_modName.Upper().c_str(), pMod->m_modName.Upper().c_str(), rlsCntBits);
		...
		m_barFuncDef.Append("\tassert_msg(c_t%d_htCtrl == HT_INVALID, \"Runtime check failed in CPers%s%s::HtBarrier%s(...)"
			...
			pMod->m_execStg, unitNameUc.c_str(), pModInst->m_instName.Uc().c_str(), barName.c_str());
		if (bMimicHtCont) {
			// mimic HtContinue
		...
		if (pMod->m_instSet.GetReplCnt(pModInst->m_instId) > 1) {
			...
			m_barIoDecl.Append("\tsc_out<bool> o_%sToBarCtl_bar%sEnter;\n",
				pMod->m_modName.Lc().c_str(), pBar->m_name.Uc().c_str());
			if (pBar->m_barIdW.AsInt() > 0)
				m_barIoDecl.Append("\tsc_out<ht_uint%d > o_%sToBarCtl_bar%sId;\n",
				pBar->m_barIdW.AsInt(), pMod->m_modName.Lc().c_str(), pBar->m_name.Uc().c_str());
			m_barIoDecl.Append("\tsc_out<sc_uint<%s_HTID_W+%d> > o_%sToBarCtl_bar%sReleaseCnt;\n",
				pMod->m_modName.Upper().c_str(), rlsCntBits, pMod->m_modName.Lc().c_str(), pBar->m_name.Uc().c_str());
			m_barIoDecl.Append("\tsc_in<bool> i_barCtlTo%s_bar%sRelease;\n",
				pMod->m_modName.Uc().c_str(), pBar->m_name.Uc().c_str());
			if (pBar->m_barIdW.AsInt() > 0)
				m_barIoDecl.Append("\tsc_in<ht_uint%d > i_barCtlTo%s_bar%sId;\n",
				pBar->m_barIdW.AsInt(), pMod->m_modName.Uc().c_str(), pBar->m_name.Uc().c_str());
			m_barIoDecl.Append("\n");
		}

		m_barRegDecl.Append("\tbool c_t%d_bar%sEnter;\n", pMod->m_execStg, pBar->m_name.Uc().c_str());
		GenModDecl(eVcdAll, m_barRegDecl, vcdModName, "bool", VA("r_t%d_bar%sEnter", pMod->m_execStg + 1, pBar->m_name.Uc().c_str()));
		barPreInstr.Append("\tc_t%d_bar%sEnter = false;\n", pMod->m_execStg, pBar->m_name.Uc().c_str());
		barReg.Append("\tr_t%d_bar%sEnter = c_t%d_bar%sEnter;\n",
			pMod->m_execStg + 1, pBar->m_name.Uc().c_str(), pMod->m_execStg, pBar->m_name.Uc().c_str());

		if (pBar->m_barIdW.AsInt() > 0) {
			m_barRegDecl.Append("\tht_uint%d c_t%d_bar%sId;\n", pBar->m_barIdW.AsInt(), pMod->m_execStg, pBar->m_name.Uc().c_str());
			GenModDecl(eVcdAll, m_barRegDecl, vcdModName, VA("ht_uint%d", pBar->m_barIdW.AsInt()), VA("r_t%d_bar%sId", pMod->m_execStg + 1, pBar->m_name.Uc().c_str()));
		...
		m_barRegDecl.Append("\tsc_uint<%s_INSTR_W> c_t%d_bar%sReleaseInstr;\n",
			pMod->m_modName.Upper().c_str(), pMod->m_execStg, pBar->m_name.Uc().c_str());
		GenModDecl(eVcdAll, m_barRegDecl, vcdModName, VA("sc_uint<%s_INSTR_W>", pMod->m_modName.Upper().c_str()),
			VA("r_t%d_bar%sReleaseInstr", pMod->m_execStg + 1, pBar->m_name.Uc().c_str()));
		...
		m_barRegDecl.Append("\tsc_uint<%s_HTID_W+%d> c_t%d_bar%sReleaseCnt;\n",
			pMod->m_modName.Upper().c_str(), rlsCntBits, pMod->m_execStg, pBar->m_name.Uc().c_str());
		GenModDecl(eVcdAll, m_barRegDecl, vcdModName, VA("sc_uint<%s_HTID_W+%d>", pMod->m_modName.Upper().c_str(), rlsCntBits),
			VA("r_t%d_bar%sReleaseCnt", pMod->m_execStg + 1, pBar->m_name.Uc().c_str()));
		...
		if (pBar->m_barIdW.AsInt() == 0) {
			...
			if (pMod->m_instSet.GetReplCnt(pModInst->m_instId) == 1) {
				...
				barPreInstr.Append("\tsc_uint<%s_HTID_W+%d> c_bar%sEnterCnt = r_bar%sEnterCnt;\n",
					pMod->m_modName.Upper().c_str(), rlsCntBits, pBar->m_name.Uc().c_str(), pBar->m_name.Uc().c_str());
			...
			if (pMod->m_instSet.GetReplCnt(pModInst->m_instId) == 1) {
				barPreInstr.Append("\tm_bar%sActive.read_addr(r_t%d_bar%sId);\n",
					pBar->m_name.Uc().c_str(), pMod->m_execStg + 1, pBar->m_name.Uc().c_str());
				...
				barPreInstr.Append("\tm_bar%sActive.read_addr(r_t%d_bar%sEnter ? r_t%d_bar%sId : r_bar%sRlsId);\n",
					pBar->m_name.Uc().c_str(), pMod->m_execStg + 1, pBar->m_name.Uc().c_str(),
					pMod->m_execStg + 1, pBar->m_name.Uc().c_str(), pBar->m_name.Uc().c_str());
			...
			if (pMod->m_instSet.GetReplCnt(pModInst->m_instId) == 1) {
				m_barRegDecl.Append("\tht_dist_ram<sc_uint<%s_HTID_W+%d>, %d> m_bar%sEnterCnt;\n",
					pMod->m_modName.Upper().c_str(), rlsCntBits, pBar->m_barIdW.AsInt(), pBar->m_name.Uc().c_str());
		...
		if (pMod->m_instSet.GetReplCnt(pModInst->m_instId) > 1 && pMod->m_threads.m_htIdW.AsInt() > 0) {
			if (pBar->m_barIdW.AsInt() > 0) {
				m_barRegDecl.Append("\tht_dist_que<ht_uint%d, %d> m_bar%sRlsIdQue;\n",
					pBar->m_barIdW.AsInt(), pMod->m_threads.m_htIdW.AsInt(), pBar->m_name.Uc().c_str());
				barReg.Append("\tm_bar%sRlsIdQue.clock(%s);\n", pBar->m_name.Uc().c_str(), reset.c_str());
			} else {
				m_barRegDecl.Append("\tsc_uint<%s_HTID_W+%d> r_bar%sRlsQueCnt;\n",
					pMod->m_modName.Upper().c_str(), rlsCntBits, pBar->m_name.Uc().c_str());
		...
		if (pBar->m_barIdW.AsInt() > 0) {
			...
			if (pMod->m_instSet.GetReplCnt(pModInst->m_instId) == 1)
				...
				pMod->m_execStg, pBar->m_name.Uc().c_str(), pBar->m_name.Uc().c_str(), pBar->m_barIdW.AsInt() - 1);
			else {
		...
		if (pMod->m_instSet.GetReplCnt(pModInst->m_instId) > 1) {
			...
			if (pMod->m_threads.m_htIdW.AsInt() > 0) {
				if (pBar->m_barIdW.AsInt() > 0)
					...
					pBar->m_name.Uc().c_str(), pMod->m_modName.Uc().c_str(), pBar->m_name.Uc().c_str());
				else
		...
		if (pMod->m_instSet.GetReplCnt(pModInst->m_instId) > 1) {
			...
			barOut.Append("\to_%sToBarCtl_bar%sEnter = r_t%d_bar%sEnter;\n",
				pMod->m_modName.Lc().c_str(), pBar->m_name.Uc().c_str(), pMod->m_execStg + 1, pBar->m_name.Uc().c_str());
			if (pBar->m_barIdW.AsInt() > 0)
	...
	if (pMod->m_instSet.GetReplCnt(pModInst->m_instId) == 1) return;

	// Generate barrier centralized control module
	string incName = VA("Pers%sBarCtl.h", pModInst->m_fileName.Uc().c_str());
	...
	int replCnt = pMod->m_instSet.GetReplCnt(pModInst->m_instId);
	...
	for (size_t barIdx = 0; barIdx < pMod->m_barrierList.size(); barIdx += 1) {
		...
		fprintf(incFile, "\tsc_in<bool> i_%sToBarCtl_bar%sEnter[%d];\n",
			pMod->m_modName.Lc().c_str(), pBar->m_name.Uc().c_str(), replCnt);
		if (pBar->m_barIdW.AsInt() > 0)
			fprintf(incFile, "\tsc_in<ht_uint%d > i_%sToBarCtl_bar%sId[%d];\n",
			pBar->m_barIdW.AsInt(), pMod->m_modName.Lc().c_str(), pBar->m_name.Uc().c_str(), replCnt);
		fprintf(incFile, "\tsc_in<sc_uint<%s_HTID_W+%d> > i_%sToBarCtl_bar%sReleaseCnt[%d];\n",
			pMod->m_modName.Upper().c_str(), rlsCntBits, pMod->m_modName.Lc().c_str(), pBar->m_name.Uc().c_str(), replCnt);
		...
		fprintf(incFile, "\t\tsc_uint<%s_HTID_W+%d> m_releaseCnt;\n", pMod->m_modName.Upper().c_str(), rlsCntBits);
		fprintf(incFile, "\t};\n");
		fprintf(incFile, "\n");

		if (pMod->m_threads.m_htIdW.AsInt() > 0)
			fprintf(incFile, "\tht_dist_que<CBar%sInfo, %d> m_bar%sInfoQue[%d];\n",
			pBar->m_name.Uc().c_str(), pMod->m_threads.m_htIdW.AsInt(), pBar->m_name.Uc().c_str(), replCnt);
		else
	...
	string cppName = VA("Pers%sBarCtl.cpp", pModInst->m_fileName.Uc().c_str());
	...
	for (size_t barIdx = 0; barIdx < pMod->m_barrierList.size(); barIdx += 1) {
		...
		if (pBar->m_barIdW.AsInt() > 0)
			...
			pBar->m_name.Uc().c_str(), pMod->m_modName.Lc().c_str(), pBar->m_name.Uc().c_str());
		...
		fprintf(cppFile, "\to_barCtlTo%s_bar%sRelease = r_bar%sRelease;\n",
			pMod->m_modName.Uc().c_str(), pBar->m_name.Uc().c_str(), pBar->m_name.Uc().c_str());
		if (pBar->m_barIdW.AsInt() > 0)
