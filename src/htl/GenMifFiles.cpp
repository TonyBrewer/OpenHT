...
void CDsnInfo::GenerateMifFiles(int mifId)
{
	...
	if (bMifRead) {
		...
		GenModDecl(eVcdAll, incCode, vcdModName, "bool", "r_mifToXbar_rdRspFull");
		fprintf(incFile, "\n");
	}
	...
	for (size_t instIdx = 0; instIdx < mifInstList.size(); instIdx += 1) {
		...
		bool bNeed_bRrHold = modMemPort.m_bMultiQwWrReq;
	...
	if (bMifRead) {
		...
		fprintf(cppFile, "\tr_mifToXbar_rdRspFull = ");
	...
	if (bMifRead)
		fprintf(cppFile, "\to_mif%dToXbar%s_rdRspFull = r_mifToXbar_rdRspFull;\n", mifId, mifName);
	else
